...
const updateStudent = (mssv, malop, holot, ten, ntns, phai, dt_sv, emailSV, faceID) => {
...
export const openAttendanceByTeacher = (tkbId) => {
    return axios.post(`${import.meta.env.VITE_BACKEND_URL}/v1/api/teacher/open-attendance`, { tkbId });
};

export const closeAttendanceByTeacher = (tkbId) => {
    return axios.post(`${import.meta.env.VITE_BACKEND_URL}/v1/api/teacher/close-attendance`, { tkbId });
};
...
// ======================================= API đăng ký tài khoản cho sinh viên và giảng viên =====================================
const registerAccount = (email, username, password, mssv, mgv) => {
...
const getFullUser = (current, pageSize) => {
    const URL_BACKEND = `/v1/api/allusers?current=${current}&pageSize=${pageSize}`;
    return axios.get(URL_BACKEND);
};

const updateUser = (id, email, username, mssv, mgv) => {
    const URL_BACKEND = `/v1/api/updateuser`;
    const userData = {
        id: id,
        email: email,
        username: username,
...
const deleteUser = (id) => {
    const URL_BACKEND = `/v1/api/deleteuser/${id}`;
    return axios.delete(URL_BACKEND);
};
