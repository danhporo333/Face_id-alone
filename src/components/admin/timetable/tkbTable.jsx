...
import { deleteTkb, assignStudentToTkb, fetchAllStudent } from '../../../services/api.service.js';
...
const TKBtable = (props) => {
    ...
    const handleAssign = async () => {
        try {
            ...
            console.log('selectedTkb.id:', selectedTkb.id);

            const promises = selectedRowKeys.map((mssv) => {
                console.log('Gọi assignStudentToTkb với:', { mssv, tkbId: selectedTkb.id });
                return assignStudentToTkb(mssv, selectedTkb.id);
            });
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, record) => {
                ...
                if (coMat > 0) {
                    statuses.push(
                        <div key="comat" style={{ color: '#52c41a', fontWeight: 'bold', fontSize: '14px' }}>
                            Có mặt: {coMat} người
                        </div>,
                    );
                }
                ...
                if (diTre > 0) {
                    ...
                    const diTreList = record.diemDanh.filter((dd) => dd.coMat && dd.diTre);
                    const lyDoList = diTreList
                        .filter((dd) => dd.lyDoKhac) // Chỉ lấy những sinh viên có lý do
                        .map((dd) => dd.lyDoKhac);

                    statuses.push(
                        <div key="ditre" style={{ color: '#fa8c16', fontWeight: 'bold', fontSize: '14px' }}>
                            <div>Đi trễ: {diTre} người</div>
                            {lyDoList.length > 0 && (
                                <div
                                    style={{
                                        fontSize: '12px',
                ...
                if (vang > 0) {
                    statuses.push(
                        <div key="vang" style={{ color: '#ff4d4f', fontWeight: 'bold', fontSize: '14px' }}>
                            Vắng: {vang} người
                        </div>,
                    );
                }

                // Thêm tổng số sinh viên
                statuses.push(
                    <div key="total" style={{ color: '#666', fontSize: '12px', marginTop: '4px' }}>
                ...
                return (
                    <div style={{ fontSize: '12px', lineHeight: '1.4' }}>
                        {statuses.length > 1 ? statuses : <span style={{ color: '#999' }}>Chưa có dữ liệu</span>}
                    </div>
                );
            },
        },
